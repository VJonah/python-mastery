...
Answer: You save even more memory! In general, wherever there are repeated strings, might be worth "caching" the string objects using intern.
...
1. If I use the interning trick the function saves just under 3 items as much memory over not using intern for all the strings. The original 'read_csv_as_dicts' took: (188382846, 188412802), the 'read_csv_as_columns' without the interning trick took:  (96177883, 96208658). And when replacing all the str with intern gave us: (34903512, 34934406).

2. And yes, all the earlier CTA analysis code works just as well!
